...
public class BTreeFile implements DbFile {
		...
		try {
			if (f == null) {
				if (pid.pgcateg() == BTreePageId.LEAF) {
					...
					BTreeInternalPageIterator itr = new BTreeInternalPageIterator(curPage);
					// itr has at least one entry
					BTreeEntry entry = itr.next();
					return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
				}

			} else {
			    // we only need to consider these 2 kinds of pages
				if (pid.pgcateg() == BTreePageId.LEAF) {
					return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
				}
				else if (pid.pgcateg() == BTreePageId.INTERNAL){
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// move data
		int num_move = page.getNumTuples() / 2;

		BTreeLeafPageReverseIterator re_itr = new BTreeLeafPageReverseIterator(page);
		while (re_itr.hasNext() && num_move > 0){
			Tuple t = re_itr.next();
			page.deleteTuple(t);
		...
		// copy up the key value
		Field key = re_itr.next().getField(keyField);
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), key);
		page.setParentId(parentPage.pid); newPage.setParentId(parentPage.pid);

		BTreeEntry newEntry = new BTreeEntry(key, page.pid, newPage.pid);
		parentPage.insertEntry(newEntry);

		// choose the right page to return
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int num_move = page.getNumEntries() / 2;
		BTreeInternalPageReverseIterator re_iter = new BTreeInternalPageReverseIterator(page);

		while(re_iter.hasNext() && num_move > 0){
			BTreeEntry entry = re_iter.next();
		...
		BTreeEntry parentEntry = new BTreeEntry(key, page.pid, newPage.pid);
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int my_entry = page.getNumTuples();
        int sibling_entry = sibling.getNumTuples();
        int move_num = (my_entry + sibling_entry) / 2 - my_entry;

        if (isRightSibling) {
            BTreeLeafPageIterator itr = new BTreeLeafPageIterator(sibling);
            Field key;
            do {
                Tuple t = itr.next();
                ...
                page.insertTuple(t);
                move_num--;
            } while (itr.hasNext() && move_num > 0);

            entry.setKey(key);
            parent.updateEntry(entry);
        }
        else{
            BTreeLeafPageReverseIterator itr = new BTreeLeafPageReverseIterator(sibling);
            while (itr.hasNext() && move_num > 0){
                Tuple t = itr.next();
                sibling.deleteTuple(t);
                page.insertTuple(t);
            ...
            Field key = itr.next().getField(keyField);
            entry.setKey(key);
            parent.updateEntry(entry);
        }
	}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// shift needed elements
		int MyEntry = page.getNumEntries();
		int leftSiblingEntry = leftSibling.getNumEntries();
		int move_num = (MyEntry + leftSiblingEntry) / 2 - MyEntry - 1;

		// first, parentEntry push down
        BTreeInternalPageReverseIterator re_itr = new BTreeInternalPageReverseIterator(leftSibling);
        BTreeInternalPageIterator itr = new BTreeInternalPageIterator(page);
        if (re_itr.hasNext() && itr.hasNext()) {
			page.insertEntry(new BTreeEntry(parentEntry.getKey(), re_itr.next().getRightChild(), itr.next().getLeftChild()));
			updateParentPointers(tid, dirtypages, page);
        }

        BTreeInternalPageReverseIterator re_itr2 = new BTreeInternalPageReverseIterator(leftSibling);
        while (re_itr2.hasNext() && move_num > 0){
            BTreeEntry entry = re_itr2.next();
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int num_move = (rightSibling.getNumEntries() + page.getNumEntries()) / 2 - page.getNumEntries() - 1;
		// first, move parent entry
		BTreeInternalPageIterator itr = new BTreeInternalPageIterator(rightSibling);
		BTreeInternalPageReverseIterator re_itr = new BTreeInternalPageReverseIterator(page);
		//if (itr.hasNext() && re_itr.hasNext()){
			BTreeEntry newLeftEntry = new BTreeEntry(parentEntry.getKey(),re_itr.next().getRightChild(), itr.next().getLeftChild());
			page.insertEntry(newLeftEntry);
			updateParentPointers(tid, dirtypages, page);
		//}

		// second, shift other elements
		BTreeInternalPageIterator itr2 = new BTreeInternalPageIterator(rightSibling);
		while (itr2.hasNext() && num_move > 0){
			BTreeEntry entry = itr2.next();
			rightSibling.deleteKeyAndLeftChild(entry);
			page.insertEntry(entry);
			num_move--;
		}

		// final, pull up the element in the right sibling
		//if (itr2.hasNext()) {
			BTreeEntry entry = itr2.next();
			rightSibling.deleteKeyAndLeftChild(entry);
			parentEntry.setKey(entry.getKey());
		...
		int move_num = (rightSibling.getNumEntries() + page.getNumEntries()) / 2 - page.getNumEntries() - 1;

       	BTreeInternalPageIterator itr2 = new BTreeInternalPageIterator(rightSibling);
        while (itr2.hasNext() && move_num > 0); {
            BTreeEntry entry = itr2.next();
            rightSibling.deleteKeyAndLeftChild(entry);
            page.insertEntry(entry);
            move_num--;
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeLeafPageIterator itr = new BTreeLeafPageIterator(rightPage);
		while (itr.hasNext()){
			Tuple t = itr.next();
			rightPage.deleteTuple(t);
			leftPage.insertTuple(t);
		}
		BTreePageId tmpRight = rightPage.getRightSiblingId();
		leftPage.setRightSiblingId(tmpRight);
        setEmptyPage(tid, dirtypages, rightPage.getId().pageNumber());
		if (tmpRight != null) {
			BTreeLeafPage rNewPage = (BTreeLeafPage) getPage(tid, dirtypages, tmpRight, Permissions.READ_WRITE);
			rNewPage.setLeftSiblingId(leftPage.getId());
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// first, push down parent entry
		BTreeInternalPageIterator right_itr = new BTreeInternalPageIterator(rightPage);
		BTreeInternalPageReverseIterator left_itr = new BTreeInternalPageReverseIterator(leftPage);
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), left_itr.next().getRightChild(), right_itr.next().getLeftChild());

		leftPage.insertEntry(entry);

		// Then shift all the other elements
		BTreeInternalPageIterator itr = new BTreeInternalPageIterator(rightPage);
		//BTreeInternalPageReverseIterator re_itr = new BTreeInternalPageReverseIterator(rightPage);

		while (itr.hasNext()){
			//assert(itr.hasNext() == re_itr.hasNext());
			entry = itr.next();
			//re_itr.next();
